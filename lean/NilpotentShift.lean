...
lemma jordanNilpotent2_mulVec (x : Fin 2 → ℂ) :
  ...
  constructor
  · simp [Matrix.mulVec, jordanNilpotent2, Fin.sum_univ_two]
  · simp [Matrix.mulVec, jordanNilpotent2, Fin.sum_univ_two]
...
lemma jordanNilpotent2_sq : jordanNilpotent2 * jordanNilpotent2 = 0 := by
  ...
  fin_cases i <;> fin_cases j <;>
    simp [jordanNilpotent2, Matrix.mul_apply, Fin.sum_univ_two]
...
lemma rayleigh_jordanNilpotent2 (x : Fin 2 → ℂ) :
  ...
  simp [dotProduct, Fin.sum_univ_two, jordanNilpotent2_mulVec]
...
lemma mul_le_mean_sq {a b : ℝ} (ha : 0 ≤ a) (hb : 0 ≤ b) :
    a * b ≤ (a ^ 2 + b ^ 2) / 2 := by
...
theorem eNorm_mulVec_le (x : Fin 2 → ℂ) :
  ...
  have : ∑ i : Fin 2, ‖jordanNilpotent2.mulVec x i‖ ^ 2 = ‖x 1‖ ^ 2 := by
    simp [Fin.sum_univ_two, h.1, h.2]
  rw [this]
...
lemma eNorm_S_e1 : eNorm (jordanNilpotent2.mulVec e1) = 1 := by
  ...
  simp [eNorm, Fin.sum_univ_two, h.1, h.2, e1]
...
lemma rayleigh_eqWeight :
    star eqWeight ⬝ᵥ jordanNilpotent2.mulVec eqWeight = (1 / 2 : ℂ) := by
  rw [rayleigh_jordanNilpotent2]
  -- both components are the same nonnegative real `(√2)⁻¹`
  have hstar : star (eqWeight 0) = eqWeight 0 := by
    simp [eqWeight, star_def, conj_ofReal]
  rw [hstar]
  simp only [eqWeight]
  have hR : ((Real.sqrt 2)⁻¹ : ℝ) * ((Real.sqrt 2)⁻¹ : ℝ) = (1 / 2 : ℝ) := by
    rw [← pow_two, inv_pow, Real.sq_sqrt (by norm_num : (0 : ℝ) ≤ 2)]
    norm_num
  have : ((↑((Real.sqrt 2)⁻¹ : ℝ) : ℂ) * ↑((Real.sqrt 2)⁻¹ : ℝ)) = ↑(1 / 2 : ℝ) := by
    rw [← ofReal_mul, hR]
...
theorem scalar_monomial_ratio_eq_two :
    (∃ x : Fin 2 → ℂ, eNorm x = 1 ∧ eNorm (jordanNilpotent2.mulVec x) = 1) ∧
      ...
      (∃ x : Fin 2 → ℂ, eNorm x = 1 ∧
        star x ⬝ᵥ jordanNilpotent2.mulVec x = (1 / 2 : ℂ)) := by
